...
@section("title", "$accreditation->title | Accreditations")
...
@section("content")
        ...
        <div class="row">
            <div class="col-6">
                <div class="content-header">
                    <h2 class="content-title">Accreditation update form</h2>
                    <div>
                        <button id="accreditationSubmitBtn" class="btn btn-md rounded font-sm hover-up">Publish</button>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <form id="accreditationForm" action="{{ route("admin.accreditations.update", $accreditation) }}" method="POST"
                            ...
                            <div class="mb-4">
                                ...
                                <img src="{{ Storage::url("accreditations/" . $accreditation->image) }}"
                                    alt="accreditation.jpg" width="200">
                            </div>

                            <div class="mb-4">
                                <label class="form-label">Description</label>
                                <textarea placeholder="Type here" class="form-control" rows="4" name="description" required>{{ $accreditation->description }}</textarea>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    @pushOnce("scripts")
        <script>
            $('#accreditationSubmitBtn').on('click', function() {
                $('#accreditationForm').submit();
            });
        </script>
    @endPushOnce
@endsection
